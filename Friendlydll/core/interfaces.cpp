...
T* GetVMT(uintptr_t address, int index, uintptr_t offset) // Address must be a VTable pointer, not a VTable !
{
	...
	int32_t relativeAddress = *(int32_t*)(instruction + step);
	uintptr_t realAddress = instruction + instructionSize + (uintptr_t)(intptr_t)relativeAddress;
...
void interfaces::Prepare() noexcept
{
	...
	const auto mhSig = mem::PatternScan("client.dll", "48 8B 0D ?? ?? ?? ?? 48 8B 01 FF 50 ?? 48 8B 03 48 8B CB FF 90 ?? ?? ?? ?? 48 8B 4B");
	if (mhSig) movehelper = *reinterpret_cast<IMoveHelper**>(mem::GetAbsAddress(mhSig));
}